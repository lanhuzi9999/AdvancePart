...
public class ApplicationData extends Application {
	...
	public static final String ACTION_JOIN = "com.tarena.xmpp.view.InputRoomFragment";
	public static final String ACTION_SHOW_GROUP_CHAT_MESSAGE = "com.tarena.xmpp.view.groupChatActivity.show";
	...
	public void onCreate() {
		...
		xmppConnection
				.addPacketListener(privateChatListener, privateChatFilter);
		new Thread() {
			public void run() {
				try {
					Log.i("login", "建立连接 threadid="
	...
	class PrivateChatFilter implements PacketFilter {
		...
		public boolean accept(Packet packet) {
			...
			if (packet instanceof Message) {
				...
				org.jivesoftware.smack.packet.Message.Type type = message
						.getType();
			...
			try {
				...
				if (from.contains("/")) {
					...
					Intent intent = new Intent(
							ApplicationData.ACTION_SHOW_PRIVATE_MESSAGE);
					sendBroadcast(intent);
				}
			} catch (Exception e) {
				// TODO: handle exception
	...
	class GroupChatListener implements PacketListener {
		...
		public void processPacket(Packet packet) {
			...
			LogUtil.i("groupChat", from + ":" + body);
			// 13082@tarena3gxmpp.com/username
			if (from.contains("/")) {
				// 判断这个messaage是不是我发的
				String msgUsername = from.substring(from.indexOf("/") + 1);
				if (!ApplicationData.currentUser.contains(msgUsername)) {
					// 别人发的
					String roomname = from.substring(0, from.indexOf("/"));
					GroupChatEntity.map.put(roomname, msg);
					// 发广播
					Intent intent = new Intent(
							ApplicationData.ACTION_SHOW_GROUP_CHAT_MESSAGE);
					sendBroadcast(intent);
				}
			}
